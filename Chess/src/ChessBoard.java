public class ChessBoard {
    ...
    public boolean moveToPosition (int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if (hasPiecesInBetween(startLine, startColumn, endLine, endColumn)) {
                    return false;
                }
    ...
    private boolean hasPiecesInBetween(int startLine, int startColumn, int endLine, int endColumn) {
        if (startLine == endLine) { // Движение по горизонтали
            for (int column = Math.min(startColumn, endColumn) + 1; column < Math.max(startColumn, endColumn); column++) {
                if (board[startLine][column] != null) {
                    return true;
                }
            }
        } else if (startColumn == endColumn) { // Движение по вертикали
            for (int line = Math.min(startLine, endLine) + 1; line < Math.max(startLine, endLine); line++) {
                if (board[line][startColumn] != null) {
                    return true;
                }
            }
        } else { // Диагональное движение
            int stepX = (endColumn - startColumn) / Math.abs(endColumn - startColumn);
            int stepY = (endLine - startLine) / Math.abs(endLine - startLine);
            for (int i = 1; i < Math.abs(endColumn - startColumn); i++) {
                if (board[startLine + i * stepY][startColumn + i * stepX] != null) {
                    return true;
                }
            }
        }
        ...
        if (nowPlayer.equals("White")) {
            ...
            if (board[0][7].getSymbol().equals("R") && board[0][4].getSymbol().equals("K") &&
                ...
                if (board[0][7].getColor().equals("White") && board[0][4].getColor().equals("White") &&
                    ...
                    board[0][4] = null;
                    board[0][5] = new King("White");
                    board[0][5].check = false;
                    board[0][7] = null;
                    board[0][6] = new Rook("White");
